...
class GoogleAdsClient(BaseIntegrationClient):
    ...
    def __init__(self):
        ...
        rate_config = RateLimitConfig(
            requests_per_minute=30, requests_per_hour=900, requests_per_day=15000
        )
        ...
        self.login_customer_id = os.getenv(
            "GOOGLE_ADS_LOGIN_CUSTOMER_ID", self.customer_id
        )
    ...
    async def fetch_campaign_performance(
        ...
        response = await self._make_request(
            "POST", url, headers=headers, json_data={"query": query}
        )
        ...
        if (
            response_batches
            and isinstance(response_batches[0], dict)
            and "campaign" in response_batches[0]
        ):
    ...
    async def update_campaign_budget(
        ...
        mutate_response = await self._make_request(
            "POST", mutate_url, headers=headers, json_data={"operations": operations}
        )
    ...
    async def _update_campaign_status(
        ...
        response = await self._make_request(
            "POST", url, headers=headers, json_data={"operations": operations}
        )
    ...
    async def get_audience_insights(
        ...
        if filters and filters.get("include_demographics"):
            ...
            demo_response = await self._make_request(
                "POST", url, headers=headers, json_data={"query": demographic_query}
            )
